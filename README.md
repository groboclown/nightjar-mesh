...
* The file [envoy.yaml.mustache](nightjar/envoy.yaml.mustache) is the template for generating the configuration for Envoy proxy.  This is refreshed every `REFRESH_TIME` seconds by processing it through the data generated by the [generate-envoy-proxy.py](nightjar/generate-envoy-proxy.py) script.  The data used to process the template file is defined in the [generation-data-schema.yaml](nightjar/generation-data-schema.yaml) data schema.
