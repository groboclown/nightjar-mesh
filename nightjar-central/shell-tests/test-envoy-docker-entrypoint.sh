...
DEBUG_CONTAINER=0 /bin/sh ${TEST_DIR}/../envoy-docker/nightjar-src/entrypoint.sh
ec=$?
if [ ${ec} -ne 0 ]; then
...
test -f "${ARGS_FILE1}" && rm "${ARGS_FILE1}"
export MOCK_EXIT_CODE1=0
DEBUG_CONTAINER=1 /bin/sh ${TEST_DIR}/../envoy-docker/nightjar-src/entrypoint.sh ${TEST_DIR}/mock-script1.sh a b c
ec=$?
if [ ${ec} -ne 0 ]; then
